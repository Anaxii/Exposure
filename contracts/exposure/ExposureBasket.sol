...
contract ExposureBasket is ERC20, Ownable {
     ...
     */
    mapping(uint256 => mapping(address => bool)) public bypassTrade;
    ...
    function burn(uint256 amount, address to) external {
        ...
        for (uint256 i = 0; i < tokens[_epoch].length; i++) {
            tokenAmount = (tokenPortions[_epoch][tokens[_epoch][i]] * (newAmount)) / 1e18;
     ...
     * - `onlyOwner`.
     */
    function initETF() external onlyOwner {
        require(!isLive);
        require(rebalanceStep == 10);
