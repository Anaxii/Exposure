...
//mapping(uint256 => uint256) private _epochPayout;
//// rebalance timers
//mapping(uint256 => uint256) public _rebalanceBlock;
//mapping(uint256 => uint256) public _rebalanceTimeStamp;
//uint256 public expenseRatio;
//uint256 public numTokensPayout;
...
//
//mapping(uint256 => uint256) feeTiers;
//mapping(address => uint256) userFeeTier;
//mapping(uint256 => uint256) private rebalancePayoutAmounts;
//mapping(uint256 => mapping(address => uint256)) private amountBought;
//mapping(uint256 => mapping(address => uint256)) private amountSold;
//
//event ETF_Exchange(address indexed from, address[] tokens, uint256[] amounts, uint256[] fees);
//// event for when rebalanceTimer is changed
//event NewRebalanceTimeConfiguration(uint256, uint256);
//// event for changing fee tiers
//event UpdatedFeeTier(uint256, uint256);
//
//
...
//// default 0.3% fee
//feeTiers[0] = 30;
//// auto buy/sell mint/redeem fee tier
//feeTiers[1] = 50;
//// admin fee tier = 30;
//feeTiers[2] = 0;
//userFeeTier[_owner] = 2;
//// revenueFee = 30;
//basketCap = 10 * 1e18;
////10 shares
//slippageFactor = 150;
...
//usdc = _usdc;
//rebalanceTimer = 1 hours;
//indexDivisor[epoch] = 100000000 * 1e18;
//
//rebalancePayoutAmounts[0] = 5;
//rebalancePayoutAmounts[1] = 5;
//rebalancePayoutAmounts[2] = 5;
//rebalancePayoutAmounts[3] = 15;
//rebalancePayoutAmounts[4] = 5;
//rebalancePayoutAmounts[5] = 15;
...
//uint256 _fee = fee(tokenAmount, feeTiers[userFeeTier[msg.sender]]);
//// fees will be paid in shares at the end
//// tokenAmount -= _fee;
...
//_mint(to, amount);
//if (_shareFee > 0)
//_mint(feeContract, _shareFee);
//}
//
//function burn(uint256 amount, address to) external {
//require(isLive);
//require(rebalanceStep == 0 || rebalanceStep > 7);
//
//uint256[] memory _amounts = new uint256[](_tokens[epoch].length);
//uint256[] memory _fees = new uint256[](_tokens[epoch].length);
//
//// fees taken in shares
//uint256 _shareFee = fee(amount, feeTiers[userFeeTier[msg.sender]]);
//uint256 newAmount = amount - _shareFee;
//_burn(to, newAmount);
//if (_shareFee > 0)
//transferFrom(msg.sender, feeContract, _shareFee);
//
//for (uint8 i = 0; i < _tokens[epoch].length; i++)
//{
//// changed to new share amount
//uint256 tokenAmount = (_tokenPortions[epoch][_tokens[epoch][i]] * (newAmount)) / 1e18;
//// uint256 _fee = fee(tokenAmount, feeTiers[userFeeTier[msg.sender]]);
//// tokenAmount -= _fee;
//// no longer needed since feeContract just gets the shares
...
//if (msg.sender == feeContract || msg.sender == owner() || epoch == 0) {
//return;
//}
//uint256 _eRatio = expenseRatio * 1e18 / 10000;
//uint256 percentageToPay = _eRatio * percentage / 1e18;
//uint256 __epochPayout = epoch - 1;
//if (rebalanceStep > 1) {
//__epochPayout = epoch;
//}
//for (uint256 i = 0; i < _tokens[__epochPayout].length; i++) {
//IERC20 _tokenToPayout = ERC20(_tokens[__epochPayout][i]);
//uint256 amountToTransfer = _tokenToPayout.balanceOf(address(this)) * percentageToPay / 1e18;
//_tokenToPayout.transfer( msg.sender, amountToTransfer);
//}
//}
//
...
//
//// check enough time has passed for rebalance || defaults to 0 on deployment
//require(block.timestamp >= _rebalanceTimeStamp[epoch]);
//_tokens[epoch + 1] = _tokens[epoch];
//
//// set minimum time before next epoch
//if (epoch == 0) {
//_rebalanceTimeStamp[epoch + 1] = block.timestamp + rebalanceTimer;
//} else {
//_rebalanceTimeStamp[epoch + 1] = _rebalanceTimeStamp[epoch] + rebalanceTimer;
//_epochPayout[epoch + 1] = (block.timestamp - rebaseTimestamp) * 1e18 / ((365 days)) * expenseRatio / 10000;
//}
//
//// keep for getting actual time between rebalances since the time check is just an increment
//rebaseTimestamp = block.timestamp;
//
//payoutExpenseRatio(_epochPayout[epoch + 1] * rebalancePayoutAmounts[rebalanceStep] / 100);
//epoch++;
//rebalanceStep = 1;
//// 2 / 00
//}
...
//payoutExpenseRatio(_epochPayout[epoch] * rebalancePayoutAmounts[rebalanceStep] / 100);
//rebalanceStep = 2;
//}
//
...
//
//rebalanceTokenIndex++;
//payoutExpenseRatio(_epochPayout[epoch] * rebalancePayoutAmounts[rebalanceStep] / 100 / _tokens[epoch].length);
...
//}
//
//function rebaseLiquidateIndividual(uint256 percentage, address token) public {
//require(rebalanceStep == 7, "Rebalance Step is not 7.");
//liquidate(token, percentage);
//}
//
//function rebaseBuyIndividual(uint256 percentage, address token) public {
//require(rebalanceStep == 8, "Rebalance Step is not 7.");
//purchase(token, percentage);
//}
//
...
//liquidate(_tokens[epoch][i], 1e18 - amountSold[epoch][_tokens[epoch][i]]);
//}
//}
//
...
//liquidate(_tokensToRemove[epoch][i], 1e18);
//}
//}
...
//{
//purchase(_tokens[epoch][i], 1e18 - amountBought[epoch][_tokens[epoch][i]]);
//}
//}
//payoutExpenseRatio(_epochPayout[epoch] * rebalancePayoutAmounts[rebalanceStep] / 100);
//rebalanceStep = 9;
...
//}
//
//function liquidate(address _token, uint256 percentage) internal {
//require(epoch > 0);
//address[] memory path;
//
//path = new address[](2);
...
//path[1] = wavax;
//
//uint256 swapAmount = _tokenSellAmount[epoch][_token] * percentage / 1e18;
//require(amountSold[epoch][_token] + percentage <= 1e18, Strings.toString(1e18 - amountSold[epoch][_token]));
...
//}
//
//function purchase(address _token, uint256 percentage) internal {
//require(epoch > 0);
//address[] memory path;
//path = new address[](2);
//path[0] = wavax;
...
//
//// buy percentage of total buy
//amountIns[0] = amountIns[0] * percentage / 1e18;
//require(amountBought[epoch][_token] + percentage <= 1e18, Strings.toString(1e18 - amountBought[epoch][_token]));
...
//}
//
//// owner can change rebalance timer
//function setRebalanceTimer(uint256 _hours) public onlyOwner {
//rebalanceTimer = _hours * (1 hours);
//emit NewRebalanceTimeConfiguration(_hours, rebalanceTimer);
//}
//
//// change or create new fee tiers
//function setFeeTier(uint256 tier, uint256 _fee) public onlyOwner {
//feeTiers[tier] = _fee;
//}
//
//// change the fee tier of a user
//function setUserFeeTier(uint256 tier, address _user) public onlyOwner {
